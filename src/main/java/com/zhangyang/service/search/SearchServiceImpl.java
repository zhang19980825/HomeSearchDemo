...
import com.zhangyang.base.HouseSort;
...
import com.zhangyang.repository.HouseDetailRepository;
import com.zhangyang.repository.HouseRepository;
import com.zhangyang.repository.HouseTagRepository;
...
public class SearchServiceImpl implements ISearchService {
        ...
        try {
            ...
            switch (message.getOperation()) {
                ...
                case HouseIndexMessage.REMOVE:
                    ...
                    break;
                default:
                    logger.warn("Not support json for" + content);
                    break;
            }
        } catch (IOException e) {
            logger.warn("Cannot parse json for" + content, e);
        }
    }
    ...
    private void createOrUpdateIndex(HouseIndexMessage message) {
        ...
        if (tags != null && !tags.isEmpty()) {
            ...
            for (HouseTag houseTag : tags) {
                tagString = tagString.append(houseTag.getName()).append(",");
            }
            houseIndexTemplate.setTag(tagString);
        }
        ...
        ServiceResult serviceResult = addressService.lbsUpLoad(baiduMapLocation.getResult(), house.getStreet() + house.getDistrict()
                , city.getCnName() + region.getCnName() + house.getStreet() + house.getDistrict(), message.getHouseId(), house.getPrice(), house.getArea());

        if (!success||!serviceResult.isSuccess()) {
            System.out.println("上传索引失败");
            this.index(message.getHouseId(),message.getRetry()+1);
        }else {
    ...
    public void index(Long houseId, int retry) {
        if (retry > HouseIndexMessage.MAX_RETRY) {
            logger.error("Retry index time over 3 for house" + houseId + "please check it!!");
            return;
        }
    ...
    public ServiceMultiResult<Long> query(RentSearch rentSearch) {
        ...
        booleanQuery.filter(QueryBuilders.termQuery(HouseIndexKey.CITY_EN_NAME, rentSearch.getCityEnName()));

        if (rentSearch.getRegionEnName() != null && !"*".equals(rentSearch.getRegionEnName())) {
            booleanQuery.filter(QueryBuilders.termQuery(HouseIndexKey.REGION_EN_NAME, rentSearch.getRegionEnName()));
        }
    ...
    @Override
    public ServiceResult<Long> arrregateDistrictHouse(String cityEnName, String regionEnName, String district) {
